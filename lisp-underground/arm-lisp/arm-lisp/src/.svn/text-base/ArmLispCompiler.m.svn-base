...
- (int) parseFormForDefun:(Form*)form//NOTE line has been reversed reversed
{
	...
	if ([self parseFormForDefunClause:[[Form alloc] initWithString:[form string]]] == 0xabba) {
		...
		if ([_machine type] == HAVE_MACHINE_ARMASM) {
			[_machine pushReturnFromLinkedBranch];
		} else {

			[_machine pushLoopGoto:l];//FIXMENOTETODO!
		}
		return 0xabba;
	}
}
...
- (int) parseFormForFuncall:(Form*)form
{
	...
	if ([s type] == (TYPE)functype) {
		...
		while (idx2 < [[form string] length] && idx < [args length]-1) {//-1 index for last ')'
			...
			//if ([[ti second] compare:@""] == 0 || [[ti second] compare:@"\n"] == 0)//FIXME
			//	break;

			//argument is not a form 
			if ([argvalue characterAtIndex:0] != '(') {

				//NOTE : using pushdefvar
			...
			//	if (idx2 >= [[form string] length]-1)
			//		break;

			} else {
			//argument is form	
				Form *f = [[Form alloc] initWithString:argvalue];
				[f reverse];
				[self parseForms:f withIndex:0];
				NSLOG2(@"SKIPFORM=%@",[f string]);
				//idx2 = [self skipFormAndWhiteSpace:form withIndex:idx2];
				//idx2 = [self skipWhiteSpace:[form string] withIndex:idx2];
				idx2 += [[f string] length];
			//	if (idx2 >= [[form string] length]-2 && [[form string] characterAtIndex:idx2] == ')')//FIXME index -2 for '))'
			//		break;
			}
		}
